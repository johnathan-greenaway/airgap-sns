...
This module listens to an email account via IMAP and relays notifications
to a designated data stream using the Airgap SNS notification system.

Usage:
    python email_notification.py --email user@example.com --password mypassword --stream email-notifications
"""
...
logger = logging.getLogger("email-notification-module")
...
try:
    ...
    # Fall back to local imports if package is not installed
    try:
        ...
        logger.error("Please ensure airgap_sns package is installed or run from project root")
        sys.exit(1)
